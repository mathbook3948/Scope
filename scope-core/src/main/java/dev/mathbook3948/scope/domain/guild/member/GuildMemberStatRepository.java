...
public interface GuildMemberStatRepository extends JpaRepository<GuildMemberStat, Long> {
    ...
    @Query("SELECT new dev.mathbook3948.scope.domain.guild.member.GuildMemberStatLatestView(s.guildId, MAX(s.createdAt)) FROM GuildMemberStat s GROUP BY s.guildId")
    List<GuildMemberStatLatestView> findLatestCreatedAtPerGuild();
